...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		sendRequest: function(action, actionData)
		{
			...
			// if($("#bx-soa-region .bx-soa-section-content").text()){
				// let adress = $("#bx-soa-region .bx-soa-section-content").text().split(':')[1].trim();
			
				// $('#soa-property-69').val(adress);
			// }
			if (!this.startLoader())
				return;
		...
		editFadeRegionContent: function(node)
		{
			...
			if (locationProperty && locationString.length){
				addedHtml += '<strong>' + BX.util.htmlspecialchars(locationProperty.NAME) + ':</strong> '
					...
					$('#soa-property-69').val(locationString);
			}
